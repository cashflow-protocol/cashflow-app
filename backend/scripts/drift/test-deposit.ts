...
async function main() {
  ...
  // 2. Fetch Drift tokens to find the vault address for USDC
  console.log('Fetching Drift tokens...');
  const tokensRes = await fetch(`${API_BASE}/earn/v1/tokens?type=drift`);
  const tokensData = await tokensRes.json();
  if (!tokensData.success) {
    console.error('Tokens request failed:', tokensData);
    return;
  }

  const driftToken = tokensData.data.find((t: any) => t.mint === MINT);
  if (!driftToken) {
    console.error('No active Drift USDC token found');
    return;
  }
  console.log(`Found Drift vault: ${driftToken.vaultAddress} (${driftToken.vaultTitle})`);

  // 3. Request unsigned deposit transaction
  console.log('Requesting Drift deposit transaction...');
  const depositRes = await fetch(`${API_BASE}/earn/v1/deposit`, {
    ...
    body: JSON.stringify({
      type: 'drift',
      mint: MINT,
      vaultAddress: driftToken.vaultAddress,
      amount: '100000', // 0.1 USDC in raw units
