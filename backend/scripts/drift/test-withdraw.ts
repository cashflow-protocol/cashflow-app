...
async function main() {
  ...
  // 2. Fetch positions to find the Drift USDC position
  ...
  const position = positionsData.data.find(
    (p: any) => p.type === 'drift' && p.mint === MINT,
  );
  if (!position) {
    console.error('No Drift USDC position found');
    return;
  }
  ...
  console.log('Requesting Drift withdraw transaction...');
  ...
  console.log('Got unsigned transaction, transactionId:', withdrawData.transactionId);
  ...
  const sendRes = await fetch(`${API_BASE}/solana/v1/send`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ transaction: signedBase64, transactionId: withdrawData.transactionId }),
  });
